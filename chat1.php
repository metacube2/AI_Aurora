    ...
    <style>
        * {
        ...
        .connection-status.status-connected {
            background: rgba(208, 240, 160, 0.35);
            color: #2f855a;
        }
    ...
    <div class="chat-container" style="display: grid;">
        ...
        <div class="chat-area">
            ...
            <div class="chat-messages-container" id="chatMessagesContainer">
                ...
                <div class="connection-status status-connecting" id="connectionStatus" role="status" aria-live="polite">
...
async function loadUsers() {
    ...
    const showPlaceholder = userListEl && state.users.length === 0;
    ...
    try {
        ...
        if (result.success) {
            ...
            userListEl.innerHTML = `<div class="error-state">${escapeHtml(result.error || 'Nutzerliste konnte nicht geladen werden.')}</div>`;
        }
    } catch (error) {
        if (showPlaceholder) {
...
async function loadMessages(userId) {
    ...
    try {
        ...
        if (result.success) {
            ...
            state.messages = [];
            state.messagesError = result.error || 'Nachrichten konnten nicht geladen werden.';
            state.messagesLoading = false;
            renderMessages();
        }
    } catch (error) {
        state.messages = [];
        state.messagesError = 'Verbindung fehlgeschlagen. Bitte versuche es erneut.';
        state.messagesLoading = false;
        renderMessages();
    }
}
...
function renderMessages() {
    ...
    if (state.messagesLoading) {
        container.innerHTML = '<div class="loading-state">Nachrichten werden geladen…</div>';
        return;
    }

    if (state.messagesError) {
        container.innerHTML = `<div class="error-state">${escapeHtml(state.messagesError)}</div>`;
        return;
    }
...
async function sendMessage() {
    ...
    try {
        ...
        alert('Nachricht konnte nicht gesendet werden. Bitte prüfe die Verbindung.');
    }
}
...
function updateConnectionStatus(message, status) {
    ...
    statusEl.classList.remove('status-connecting', 'status-connected', 'status-error', 'is-hidden');
...
function startSSE() {
    ...
    updateConnectionStatus('Verbindung wird aufgebaut…', 'connecting');

    state.eventSource = new EventSource(`?stream=events&last_message_id=${state.lastMessageId}`);

    state.eventSource.onopen = () => {
        updateConnectionStatus('Live-Verbindung aktiv', 'connected');
    };

    state.eventSource.onerror = () => {
        updateConnectionStatus('Verbindung unterbrochen – versuche erneut…', 'error');
        if (state.eventSource) {
            state.eventSource.close();
            state.eventSource = null;
        }
